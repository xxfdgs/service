...
class GPSModel(torch.nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        ###
        # list_FC_layers = [nn.Linear(64, 256, bias=True), nn.Linear(256, 512, bias=True),
        #                   nn.Linear(512, dim_out, bias=True)]
        self.fc_ratio = nn.Linear(1, ((cfg.gt.dim_hidden) *2))  ###  比例的特征层
        list_FC_layers = [nn.Linear(((cfg.gt.dim_hidden) *4), 256, bias=True), nn.Linear(256, 512, bias=True),
                          nn.Linear(512, dim_out, bias=True)]
    ...
    def forward(self, data1, data2):
        ...
        # rep1 = self.gnn1(data1)
        # rep2 = self.gnn2(data2)
        ...
        ###
        # 将比例作为特征
        ratio_features = self.fc_ratio((rep2.ratio).unsqueeze(1))  # 调整维度

        # alpha = torch.nn.Parameter(torch.Tensor([initial_value]))
        alpha = ((rep2.ratio).unsqueeze(1)).expand(-1, graph_emb_1.shape[1])
        graph_emb = torch.cat([graph_emb_1,  alpha * graph_emb_2], dim=-1)
        ###
        ...
        graph_emb = torch.cat((graph_emb, ratio_features), dim=1)
