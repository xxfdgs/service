...
@register_network('GPSDoubleModel_multi')
class GPSModel(torch.nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        ###  torch.cat version
        list_FC_layers = [nn.Linear(((cfg.gt.dim_hidden) *2), 256, bias=True), nn.Linear(256, 512, bias=True),
                          nn.Linear(512, dim_out, bias=True)]
        list_FC_layers_2 = [nn.Linear(((cfg.gt.dim_hidden) * 2), 256, bias=True), nn.Linear(256, 512, bias=True),
                          nn.Linear(512, dim_out, bias=True)]
        list_FC_layers_3 = [nn.Linear(((cfg.gt.dim_hidden) * 2), 256, bias=True), nn.Linear(256, 512, bias=True),
                          nn.Linear(512, dim_out, bias=True)]
        #### sum verision
        # list_FC_layers = [nn.Linear((cfg.gt.dim_hidden), 256, bias=True), nn.Linear(256, 512, bias=True),
        #                  nn.Linear(512, dim_out, bias=True)]
        self.FC_layers = nn.ModuleList(list_FC_layers)
        self.FC_layers_2 = nn.ModuleList(list_FC_layers_2)
        self.FC_layers_3 = nn.ModuleList(list_FC_layers_3)
    ...
    def forward(self, data1, data2):
        ...
        graph_emb_2 = graph_emb
        for l in range(3):
            graph_emb_2 = self.FC_layers_2[l](graph_emb_2)
            graph_emb_2 = self.activation()(graph_emb_2)
        graph_emb_3 = graph_emb
        for l in range(3):
            graph_emb_3 = self.FC_layers_3[l](graph_emb_3)
            graph_emb_3 = self.activation()(graph_emb_3)
        # Final prediction
