...
@register_network('GPSDoubleModel_l1_ntx_v31_multi2_cat')
class GPSModel(torch.nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        list_FC_layers = [
            ...
            nn.Linear(512, 2, bias=True)  # dim_out=4
        ]

        # 修改2：中间预测层输出维度从 1 改为 4（对应4性质）
        list_FC_layers_middle_2mlp = [nn.Linear(256, 2, bias=True)]

        # 修改3：GPS直接预测层输出维度从 1 改为 4（对应4性质）
        list_FC_layers_2mlp = [nn.Linear(input_dim, 2, bias=True)]
    ...
    #     # 这里要求 batch.graph_feature 是 [B,4]，batch.y 也应是 [B,4]
    ...
    def forward(self, data1, data2, data3, data4, data5):
        ...
        # 步骤7：GPS直接预测和中间层预测（两者也输出 [B,2]）
        graph_emb_2mlp = self.FC_layers_2mlp[0](graph_emb_input)     # [B,2]
        graph_emb_2mlp = self.activation()(graph_emb_2mlp)
        graph_emb_middle_2mlp = self.FC_layers_midle_mlp[0](graph_emb_feature_middle)  # [B,2]
        graph_emb_middle_2mlp = self.activation()(graph_emb_middle_2mlp)

        # 步骤8：最终预测（对4个性质逐元素融合；权重仍是 [B,1] 自动broadcast到 [B,2]）
