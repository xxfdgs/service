...
def init_model_from_pretrained(model, pretrained_dir, ave_pretrained_model,
    ...
    #
    #     pretrained_dict = {k: v for k, v in ckpt[MODEL_STATE].items() if
    #                        k in model_dict and not k.startswith('model.FC_layers.')}
    #     pretrained_dict1 = {k: v for k, v in ckpt1[MODEL_STATE].items() if
    #                        k in model_dict1 and not k.startswith('model.FC_layers.')}
    #     pretrained_dict2 = {k: v for k, v in ckpt2[MODEL_STATE].items() if
    #                        k in model_dict2 and not k.startswith('model.FC_layers.')}
    #     pretrained_dict3 = {k: v for k, v in ckpt3[MODEL_STATE].items() if
    #                        k in model_dict3 and not k.startswith('model.FC_layers.')}
    #     pretrained_dict4 = {k: v for k, v in ckpt4[MODEL_STATE].items() if
    #                        k in model_dict4 and not k.startswith('model.FC_layers.')}
    #     pretrained_dict5 = {k: v for k, v in ckpt5[MODEL_STATE].items() if
    #                        k in model_dict5 and not k.startswith('model.FC_layers.')}
    #     pretrained_dict6 = {k: v for k, v in ckpt6[MODEL_STATE].items() if
    #                        k in model_dict6 and not k.startswith('model.FC_layers.')}
    #     pretrained_dict7 = {k: v for k, v in ckpt7[MODEL_STATE].items() if
    #                        k in model_dict7 and not k.startswith('model.FC_layers.')}
    #     pretrained_dict8 = {k: v for k, v in ckpt8[MODEL_STATE].items() if
    #                        k in model_dict8 and not k.startswith('model.FC_layers.')}
    #     pretrained_dict9 = {k: v for k, v in ckpt9[MODEL_STATE].items() if
    #                        k in model_dict9 and not k.startswith('model.FC_layers.')}
    #
    #     # if reset_prediction_head:
    #     #     # Filter out prediction head parameter keys.
    #     #     logging.info(f"[*] reset_prediction_head: {reset_prediction_head}")
