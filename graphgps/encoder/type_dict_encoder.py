...
@register_node_encoder('CoarseTypeDictNode')
class CoarseTypeDictNodeEncoder(torch.nn.Module):
    """Encode atom type plus the length of an optional coarse carbon-chain node."""

    def __init__(self, emb_dim):
        super().__init__()
        self.encoder = torch.nn.Embedding(
            num_embeddings=cfg.dataset.node_encoder_num_types,
            embedding_dim=emb_dim,
        )
        self.chain_length_encoder = torch.nn.Linear(1, emb_dim, bias=False)

    def forward(self, batch):
        atom_embedding = self.encoder(batch.x[:, 0])
        chain_length = batch.x[:, 9:10].float()
        chain_length = chain_length / cfg.coarse_grain_chain_length_scale
        batch.x = atom_embedding + self.chain_length_encoder(chain_length)
        return batch
