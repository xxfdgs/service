...
from graphgps.lrx_add.fds.util import calibrate_mean_var
...
class FDS(nn.Module):
    ...
    def _get_bucket_idx(self, label):
        ...
        _, bins_edges = np.histogram(a=np.array([], dtype=np.float32), bins=self.bucket_num, range=(0., 600))
        if label == 5.:
    ...
    def smooth(self, features, input_labels, epoch):
        if epoch < self.start_smooth:
            return features

        labels = input_labels.detach().to('cpu', non_blocking=True)
        buckets = np.array([self._get_bucket_idx(label) for label in labels])
        new_features = features.clone()
        for bucket in np.unique(buckets):
            new_features[torch.tensor((buckets == bucket))] = calibrate_mean_var(
                new_features[torch.tensor((buckets == bucket))],
