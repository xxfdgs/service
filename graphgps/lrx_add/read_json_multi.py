...
def plot_mae_figures_6(epochs, train_mae_dict, val_mae_dict, test_mae_dict, save_path_mae, num):
    """
    绘制MAE类指标图片（分两组）
...
def plot_mae_figures_2(epochs, train_mae_dict, val_mae_dict, test_mae_dict, save_path_mae, num):
    """
    绘制MAE类指标图片（在一张图片中显示2个子图，1行2列）
    子图1：Norm_before_mae（Train=黑, Val=蓝, Test=红）
    子图2：Norm_after_mae（Train=黑, Val=蓝, Test=红）
    """
    # 创建1个画布 + 1行2列的2个子图，设置画布大小
    fig, axes = plt.subplots(1, 2, figsize=(18, 8))  # 宽18高8，适配2个子图
    fig.suptitle('MAE Curves for Norm_before & Norm_after', fontsize=18, y=0.98)  # 整体标题

    # ========== 子图1：Norm_before_mae (第1行第1列) ==========
    ax1 = axes[0]
    # 固定颜色：Train=黑色(ko)、Val=蓝色(b-)、Test=红色(r-)
    ax1.plot(epochs, train_mae_dict['Norm_before'], 'k-', label='Train Norm_before_mae', markersize=4)
    ax1.plot(epochs, val_mae_dict['Norm_before'], 'b-', label='Val Norm_before_mae', linewidth=2)
    ax1.plot(epochs, test_mae_dict['Norm_before'], 'r-', label='Test Norm_before_mae', linewidth=2)
    ax1.set_title('Norm_before MAE', fontsize=14)
    ax1.set_xlabel('Epochs', fontsize=12)
    ax1.set_ylabel('MAE', fontsize=12)
    ax1.legend(fontsize=10)
    ax1.grid(alpha=0.3)

    # ========== 子图2：Norm_after_mae (第1行第2列) ==========
    ax2 = axes[1]
    # 固定颜色：Train=黑色(ko)、Val=蓝色(b-)、Test=红色(r-)
    ax2.plot(epochs, train_mae_dict['Norm_after'], 'k-', label='Train Norm_after_mae', markersize=4)
    ax2.plot(epochs, val_mae_dict['Norm_after'], 'b-', label='Val Norm_after_mae', linewidth=2)
    ax2.plot(epochs, test_mae_dict['Norm_after'], 'r-', label='Test Norm_after_mae', linewidth=2)
    ax2.set_title('Norm_after MAE', fontsize=14)
    ...
    # 调整子图间距，避免标题/标签重叠
    plt.tight_layout(rect=[0, 0, 1, 0.96])  # rect预留顶部标题空间

    # 保存合并后的图片（替换文件名标识）
    merged_save_path = save_path_mae.replace(f'result_{num}.png', f'mae_norm_2props_{num}.png')
    # 高分辨率保存，避免内容裁剪
    plt.savefig(merged_save_path, dpi=300, bbox_inches='tight')
    plt.close(fig)  # 关闭画布释放内存
    print(f'2个Norm指标的MAE合并图片已保存至: {merged_save_path}')

def plot_mae_figures_4(epochs, train_mae_dict, val_mae_dict, test_mae_dict, save_path_mae, num):
    """
    绘制MAE类指标图片（在一张图片中显示4个子图，分别对应4个指标）
    子图布局：2行2列
    子图1：EE_before_mae
    子图2：EE_after_mae
    子图3：Aero_Efficiency_mae
    子图4：Recovery_Efficiency_mae
    """
    # 创建1个画布 + 2行2列的4个子图，设置整体画布大小
    fig, axes = plt.subplots(2, 2, figsize=(20, 16))  # 宽20，高16，适配4个子图
    fig.suptitle('MAE Curves for All Properties', fontsize=20, y=0.98)  # 整体标题

    # ========== 子图1：EE_before_mae (第1行第1列) ==========
    ax1 = axes[0, 0]
    ax1.plot(epochs, train_mae_dict['EE_before'], 'k-', label='Train', markersize=4)
    ax1.plot(epochs, val_mae_dict['EE_before'], 'b-', label='Val', linewidth=2)
    ax1.plot(epochs, test_mae_dict['EE_before'], 'r-', label='Test', linewidth=2)
    ax1.set_title('EE_before MAE', fontsize=16)
    ax1.set_xlabel('Epochs', fontsize=12)
    ax1.set_ylabel('MAE', fontsize=12)
    ax1.legend(fontsize=10)
    ax1.grid(alpha=0.3)

    # ========== 子图2：EE_after_mae (第1行第2列) ==========
    ax2 = axes[0, 1]
    ax2.plot(epochs, train_mae_dict['EE_after'], 'k-', label='Train', markersize=4)
    ax2.plot(epochs, val_mae_dict['EE_after'], 'b-', label='Val', linewidth=2)
    ax2.plot(epochs, test_mae_dict['EE_after'], 'r-', label='Test', linewidth=2)
    ax2.set_title('EE_after MAE', fontsize=16)
    ax2.set_xlabel('Epochs', fontsize=12)
    ax2.set_ylabel('MAE', fontsize=12)
    ax2.legend(fontsize=10)
    ax2.grid(alpha=0.3)

    # ========== 子图3：Aero_Efficiency_mae (第2行第1列) ==========
    ax3 = axes[1, 0]
    ax3.plot(epochs, train_mae_dict['Aero_Efficiency'], 'k-', label='Train', markersize=4)
    ax3.plot(epochs, val_mae_dict['Aero_Efficiency'], 'b-', label='Val', linewidth=2)
    ax3.plot(epochs, test_mae_dict['Aero_Efficiency'], 'r-', label='Test', linewidth=2)
    ax3.set_title('Aerosolization_Efficiency MAE', fontsize=16)
    ax3.set_xlabel('Epochs', fontsize=12)
    ax3.set_ylabel('MAE', fontsize=12)
    ax3.legend(fontsize=10)
    ax3.grid(alpha=0.3)

    # ========== 子图4：Recovery_Efficiency_mae (第2行第2列) ==========
    ax4 = axes[1, 1]
    ax4.plot(epochs, train_mae_dict['Recovery_Efficiency'], 'k-', label='Train', markersize=4)
    ax4.plot(epochs, val_mae_dict['Recovery_Efficiency'], 'b-', label='Val', linewidth=2)
    ax4.plot(epochs, test_mae_dict['Recovery_Efficiency'], 'r-', label='Test', linewidth=2)
    ax4.set_title('mRNA_Recovery_Efficiency MAE', fontsize=16)
    ...
    # 保存合并后的图片
    merged_save_path = save_path_mae.replace(f'result_{num}.png', f'mae_all_4props_{num}.png')
    plt.savefig(merged_save_path, dpi=300, bbox_inches='tight')  # dpi提高清晰度，bbox_inches避免内容被裁剪
    plt.close(fig)  # 关闭画布，释放内存
    print(f'4个指标的MAE合并图片已保存至: {merged_save_path}')
def result_picture(path_start,read_name,repeat_num,picture_list,metric_best):
    ...
    for num in range(repeat_num):
        ...
        if picture_list == 6:
            ...
            # 解包数据
            val_loss_list, val_EE_before, val_EE_after, val_Aero, val_Recovery = val_data
            train_loss_list, train_EE_before, train_EE_after, train_Aero, train_Recovery = train_data
            test_loss_list, test_EE_before, test_EE_after, test_Aero, test_Recovery = test_data
            # val_loss_list, val_EE_before, val_EE_after, val_Aero, val_Recovery, val_Norm_before, val_Norm_after = val_data
            # train_loss_list, train_EE_before, train_EE_after, train_Aero, train_Recovery, train_Norm_before, train_Norm_after = train_data
            # test_loss_list, test_EE_before, test_EE_after, test_Aero, test_Recovery, test_Norm_before, test_Norm_after = test_data
            # 整理MAE数据为字典（方便调用）
            ...
            save_path = path_start + read_name + '/' + 'result_'+ str(num) +'.png'
            plot_loss_figure(epochs, train_loss_list, val_loss_list, test_loss_list, save_path, num)
            plot_mae_figures_4(epochs,
                             {'EE_before':train_EE_before, 'EE_after':train_EE_after, 'Aero_Efficiency':train_Aero,
                              'Recovery_Efficiency':train_Recovery},
                             val_mae_dict, test_mae_dict, save_path, num)
            ...
            # 解包数据
            val_loss_list,  val_Norm_before, val_Norm_after = val_data
            train_loss_list,  train_Norm_before, train_Norm_after = train_data
            test_loss_list,  test_Norm_before, test_Norm_after = test_data
            # 整理MAE数据为字典（方便调用）
            val_mae_dict = {
                'Norm_before': val_Norm_before,
                'Norm_after': val_Norm_after
            ...
            save_path = path_start + read_name + '/' + 'result_'+ str(num) +'.png'
            plot_loss_figure(epochs, train_loss_list, val_loss_list, test_loss_list, save_path, num)
            plot_mae_figures_2(epochs,
                             {'Norm_before':train_Norm_before, 'Norm_after':train_Norm_after},
                             val_mae_dict, test_mae_dict, save_path, num)
        elif picture_list == 1:
            # 仅绘制Loss（兼容原有逻辑）
...
if __name__ == '__main__':
    metric_best = 'loss'
    read_name = 'direct_layer2_multi4_cat_v0_batch32_lr001'
    read_path = '/home/lrx/dataset/cooperation/gps/results/'
    repeat_num = 10
    picture_list = 4  # 2 4 6
