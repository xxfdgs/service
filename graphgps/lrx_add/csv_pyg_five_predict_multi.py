...
def smiles_to_data_5(smiles_item, label_pair_list,ratio_pair_list,
    ...
    global _CURRENT_MORDRED_FEATURE
        ...
        try:
            ...
            if property_num == 4:
                ...
                if idx_sum ==0:
                    print('-')
                if num == 0:
                    data_1 = Data(x, edge_index, edge_attr, y, y1=y1, y2=y2, y3=y3, ratio=ratio_pair_list[num], mask=mask_, sum=idx_sum, aux_feat=aux_feat)
                    data_sum_1.append(data_1)
                elif num == 1:
                ...
                if num == 0:
                    data_1 = Data(x, edge_index, edge_attr, y, y1=y1, ratio=ratio_pair_list[num], mask=mask_, sum=idx_sum, aux_feat=aux_feat)
                    data_sum_1.append(data_1)
                elif num == 1:
...
class LRX_five_predict_multi(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None,subset: bool = False,
        ...
        super(LRX_five_predict_multi, self).__init__(root, transform, pre_transform,pre_filter)
    ...
    def process(self):
        ...
        if cfg.result_out == False:
            ...
            valid_data = data#[:2000]#[9000:]#[6000:9000]
            test_data = data[:cfg.train.batch_size]
        elif cfg.result_out == True:
