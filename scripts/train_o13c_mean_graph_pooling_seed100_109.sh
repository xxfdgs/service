#!/usr/bin/env bash
# O13-C: frozen O12 protocol with only GraphGPS graph pooling add -> mean.
set -euo pipefail
...
RANDOM_RUNS_ROOT="${RANDOM_RUNS_ROOT:-results/input_graphgps_optimization/o13c_mean_graph_pooling_random_seed100_109}"
OOD_RUNS_ROOT="${OOD_RUNS_ROOT:-results/input_graphgps_optimization/o13c_mean_graph_pooling_fifth_identity_ood_seed100_109_no_mordred}"
AUDIT_ROOT="${AUDIT_ROOT:-results/input_graphgps_optimization/o13c_mean_graph_pooling_config_audit}"
MAX_EPOCHS="${MAX_EPOCHS:-300}"
EARLY_STOP_PATIENCE="${EARLY_STOP_PATIENCE:-50}"
# This local training variant intentionally uses three GraphGPS layers. It is
# therefore an O13-C mean-pooling training run, not the strict one-variable
# O12-vs-O13-C comparison. Keep audits/evaluation/comparison opt-in so a
# completed checkpoint is not discarded merely because it is incomparable to
# the frozen two-layer O12 baseline.
RUN_CONFIG_AUDIT="${RUN_CONFIG_AUDIT:-0}"
...
if [[ "$RUN_CONFIG_AUDIT" == 1 ]]; then
    for target_group in "${TARGET_GROUPS[@]}"; do
        ...
        "$PYTHON" scripts/diagnostics/audit_o13c_mean_pooling_config.py \
            --baseline-effective "$baseline_effective" \
            --locked-input-csv "$INPUT_CSV" --locked-mordred "$MORDRED" \
            --output "$AUDIT_ROOT/preflight_${target_group}.json"
    done
fi
...
run_protocol() {
    ...
    for target_group in "${TARGET_GROUPS[@]}"; do
        local output_activation="identity" candidate_prefix="O13C${protocol}Norm2"
        [[ "$target_group" == "core4" ]] && output_activation="sigmoid" && candidate_prefix="O13C${protocol}Core4"
        ...
        for split_seed in "${SPLIT_SEEDS[@]}"; do
            ...
            if [[ -e "$run_dir" ]]; then
                if [[ -f "$run_dir/summary.json" && -f "$run_dir/predictions.csv" && \
                      ...
                      ( "$RUN_CONFIG_AUDIT" != 1 || -f "$run_dir/config_diff_audit.json" ) ]]; then
                    echo "Skipping completed O13-C ${protocol} ${target_group} split ${split_seed}"
                    continue
                fi
                echo "Refusing incomplete O13-C directory: $run_dir" >&2
                echo "Use a fresh RANDOM_RUNS_ROOT/OOD_RUNS_ROOT. Do not reuse an old processed cache." >&2
                exit 1
            fi
            echo "Training O13-C ${protocol} ${target_group} split ${split_seed}"
            "$PYTHON" scripts/diagnostics/run_fusion_head_experiment.py \
                ...
                --graph-pooling mean --output-activation "$output_activation" \
                ...
                2>&1 | tee "$runs_root/logs/O13C_${protocol}_${target_group}_split${split_seed}.log"
            if [[ "$RUN_CONFIG_AUDIT" == 1 ]]; then
                "$PYTHON" scripts/diagnostics/audit_o13c_mean_pooling_config.py \
                    --baseline-effective "$baseline_effective" \
                    --candidate-effective "$run_dir/effective_config.yaml" \
                    --locked-input-csv "$INPUT_CSV" --locked-mordred "$MORDRED" \
...
# Intentionally absent: fifth-only fusion, Fifth_class/identity embeddings,
# fifth ratio modulation, ratio polynomials, pooling/fusion/descriptor changes.
...
"$PYTHON" scripts/diagnostics/compare_o13c_mean_pooling.py \
    --input-csv "$INPUT_CSV" \
    ...
    --o13c-random-metrics "$RANDOM_RUNS_ROOT/corresponding_split_single_inference/metrics_by_checkpoint_target.csv" \
    --o13c-random-predictions "$RANDOM_RUNS_ROOT/corresponding_split_single_inference/predictions_by_checkpoint.csv" \
    --o13c-ood-metrics "$OOD_RUNS_ROOT/corresponding_split_single_inference/metrics_by_checkpoint_target.csv" \
    --o13c-ood-predictions "$OOD_RUNS_ROOT/corresponding_split_single_inference/predictions_by_checkpoint.csv" \
    --output-dir results/input_graphgps_optimization/o13c_mean_graph_pooling_comparison
fi
