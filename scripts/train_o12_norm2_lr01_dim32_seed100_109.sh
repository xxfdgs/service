#!/usr/bin/env bash
# Fixed-lr O12 norm2 candidate: 32-D fifth-component GraphGPS, 2 GPS layers.
set -euo pipefail
...
RUNS_ROOT="${RUNS_ROOT:-results/input_graphgps_optimization/o12_norm2_lr01_dim32_rwse16_seed100_109}"
...
for split_seed in {100..109}; do
    ...
    echo "Training split${split_seed}: lr=0.1, dim=32, gps_layers=2, rwse_dim=16"
    "$PYTHON_BIN" scripts/diagnostics/run_fusion_head_experiment.py \
        ...
        --candidate "O12LR01D32L2S${split_seed}" \
        --fusion-type concat_mlp --head-type baseline --model-type OneHotEmbedGPS \
        --seed 43 --base-lr 0.1 --warmup-epochs 50 --weight-decay 1e-5 \
        --gps-layers 2 --graph-hidden-dim 32 --rwse-dim 16 \
