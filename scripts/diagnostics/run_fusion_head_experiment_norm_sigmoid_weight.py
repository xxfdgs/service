...
def elementwise_regression_loss(prediction, labels, loss_type="mae", huber_beta=0.1):
    """Return unreduced regression loss with the same shape as ``prediction``."""
    if loss_type == "mae":
        return torch.abs(prediction - labels)
    if loss_type == "mse":
        return (prediction - labels).square()
    if loss_type == "huber":
        return functional.smooth_l1_loss(
            prediction, labels, beta=huber_beta, reduction="none"
        )
    raise ValueError(f"Unsupported loss type: {loss_type}")


def norm_sigmoid_weighted_regression_loss(
    prediction,
    transformed_labels,
    raw_labels,
    threshold,
    ...
    """Smoothly upweight raw Norm > threshold and downweight Norm < threshold.

    The weight is computed in raw physical Norm units:

        w_raw = low + (high - low) * sigmoid((y_raw - threshold) / tau)

    The minibatch weights are then normalized to mean 1 so enabling this
    option changes relative sample importance without systematically changing
    the overall gradient scale.  The regression residual itself stays in the
    model's active training space (identity/log1p and optional train-only
    z-score), so this remains compatible with target transforms.
    """
    if tau <= 0:
        raise ValueError("Norm sigmoid weighting tau must be positive.")
    if low_weight <= 0 or high_weight <= 0:
        raise ValueError("Norm sigmoid weights must be positive.")
    if high_weight <= low_weight:
        raise ValueError("Norm sigmoid high weight must exceed low weight.")

    per_element = elementwise_regression_loss(
        prediction, transformed_labels, loss_type, huber_beta
    )
    raw = raw_labels.reshape_as(per_element).to(
        dtype=per_element.dtype, device=per_element.device
    )
    weights = low_weight + (high_weight - low_weight) * torch.sigmoid(
        (raw - threshold) / tau
    )
    # Labels do not require gradients, but detach explicitly to make this
    # weighting policy impossible to backpropagate through by accident.
...
def main():
    ...
    parser.add_argument('--enable-norm-sigmoid-weighting', action='store_true',
                        help='Enable smooth target-dependent regression weighting for a single Norm target. '
                             ...
                             'values above it approach --norm-weight-high. Minibatch weights are '
                             'renormalized to mean 1. Disabled by default.')
    parser.add_argument('--norm-weight-low', type=float, default=0.5,
                        help='Lower asymptotic regression weight used by '
                             '--enable-norm-sigmoid-weighting (default: 0.5).')
    parser.add_argument('--norm-weight-high', type=float, default=2.0,
                        help='Upper asymptotic regression weight used by '
                             '--enable-norm-sigmoid-weighting (default: 2.0).')
    parser.add_argument('--norm-weight-tau', type=float, default=0.15,
                        help='Smooth transition width in raw Norm units for '
                             '--enable-norm-sigmoid-weighting (default: 0.15).')
    ...
    if args.enable_norm_sigmoid_weighting:
        if args.single_target not in {'Norm_before', 'Norm_after'} or target_count != 1:
            raise ValueError(
                '--enable-norm-sigmoid-weighting requires '
                '--single-target Norm_before or Norm_after.')
        if args.norm_threshold <= 0:
            raise ValueError('--norm-threshold must be positive.')
        if args.norm_weight_low <= 0 or args.norm_weight_high <= 0:
            raise ValueError('--norm-weight-low and --norm-weight-high must be positive.')
        if args.norm_weight_high <= args.norm_weight_low:
            raise ValueError('--norm-weight-high must be greater than --norm-weight-low.')
        if args.norm_weight_tau <= 0:
            raise ValueError('--norm-weight-tau must be positive.')
        if args.norm_positive_reg_weight != 1.0:
            raise ValueError(
                '--enable-norm-sigmoid-weighting cannot be combined with '
                '--norm-positive-reg-weight != 1.0; use one regression weighting '
                'policy at a time for an interpretable ablation.')
    ...
    cfg.enable_norm_sigmoid_weighting = bool(args.enable_norm_sigmoid_weighting)
    cfg.norm_weight_low = float(args.norm_weight_low)
    cfg.norm_weight_high = float(args.norm_weight_high)
    cfg.norm_weight_tau = float(args.norm_weight_tau)
    for key, value in {
    ...
    if not args.resume:
        ...
        (run_dir / 'run_settings.json').write_text(json.dumps({
            ...
            'enable_norm_sigmoid_weighting': bool(args.enable_norm_sigmoid_weighting),
            'norm_weight_low': float(args.norm_weight_low),
            'norm_weight_high': float(args.norm_weight_high),
            'norm_weight_tau': float(args.norm_weight_tau),
            'norm_sigmoid_weight_normalization': 'minibatch_mean_1',
    ...
    if args.resume:
        saved_run_settings_path = run_dir / 'run_settings.json'
        if not saved_run_settings_path.is_file():
            raise FileNotFoundError(
                f'Resume requested but run settings are missing: {saved_run_settings_path}')
        saved_run_settings = json.loads(
            saved_run_settings_path.read_text(encoding='utf-8'))
        saved_sigmoid_enabled = bool(
            saved_run_settings.get('enable_norm_sigmoid_weighting', False))
        if saved_sigmoid_enabled != bool(args.enable_norm_sigmoid_weighting):
            raise RuntimeError(
                'Resume would change --enable-norm-sigmoid-weighting; '
                f'saved={saved_sigmoid_enabled}, requested='
                f'{bool(args.enable_norm_sigmoid_weighting)}.')
        if args.enable_norm_sigmoid_weighting:
            resume_weight_fields = {
                'norm_threshold': float(args.norm_threshold),
                'norm_weight_low': float(args.norm_weight_low),
                'norm_weight_high': float(args.norm_weight_high),
                'norm_weight_tau': float(args.norm_weight_tau),
            }
            for field, requested in resume_weight_fields.items():
                if field not in saved_run_settings:
                    raise RuntimeError(
                        f'Resume run is missing sigmoid-weight provenance field: {field}')
                saved = float(saved_run_settings[field])
                if abs(saved - requested) > max(1e-12, abs(saved) * 1e-9):
                    raise RuntimeError(
                        'Resume would change sigmoid-weighting provenance for '
    ...
    if args.norm_threshold_reporting:
        ...
        if not args.resume:
            (run_dir / 'norm_threshold_audit.json').write_text(json.dumps({
                ...
                'norm_sigmoid_weighting_enabled': bool(args.enable_norm_sigmoid_weighting),
                'norm_sigmoid_weight_formula': (
                    'w_raw = low + (high-low) * sigmoid((raw_norm-threshold)/tau); '
                    'w = w_raw / mean_minibatch(w_raw)'
                    if args.enable_norm_sigmoid_weighting else None
                ),
                'norm_weight_low': float(args.norm_weight_low),
                'norm_weight_high': float(args.norm_weight_high),
                'norm_weight_tau': float(args.norm_weight_tau),
    ...
    for epoch in epoch_iterator:
        ...
        for batch_index, batches in enumerate(zip(*[group[0] for group in loaders])):
            ...
            if args.enable_norm_threshold_aware or args.enable_norm_sigmoid_weighting \
                    or args.norm_positive_reg_weight != 1.0 \
                    or args.norm_underprediction_weight != 0.0:
                ...
                # The smooth target-dependent weighting is intentionally
                # independent of the O14 double-only positive regression
                # weighting.  Validation above forbids enabling both policies
                # simultaneously, which keeps ablations interpretable.
                if args.enable_norm_sigmoid_weighting:
                    regression_loss = norm_sigmoid_weighted_regression_loss(
                        model_prediction,
                        transformed_label,
                        label,
                        args.norm_threshold,
                        args.norm_weight_low,
                        args.norm_weight_high,
                        args.norm_weight_tau,
                        args.training_loss,
                        args.huber_beta,
                    )
                else:
                ...
                needs_double_mask = (
                    args.norm_underprediction_weight != 0.0
                    or (not args.enable_norm_sigmoid_weighting
                        and args.norm_positive_reg_weight != 1.0)
                )
                if needs_double_mask and double_mask is None:
                    class_ids = batches[4].fifth_class_id.view(-1).long()
                    double_mask = class_ids.eq(int(double_class_id))

                if args.norm_underprediction_weight != 0.0:
                    underprediction_loss = double_high_underprediction_loss(
                        raw_prediction, label, double_mask, args.norm_threshold)
                else:
                    underprediction_loss = raw_prediction.new_zeros(())

                if args.enable_norm_threshold_aware:
                    if high_logit is None:
                        raise RuntimeError(
                            'O14-A enabled but the model did not return a high-Norm logit.')
