...
def create_loader_j():
    ...
    if cfg.dataset.task == 'graph':
        ...
        loaders_j = [
            get_loader(dataset_j[id], cfg.train.sampler, cfg.train.batch_size,
                       shuffle=False)
        ]
        delattr(dataset_j.data, 'train_graph_index')
    else:
        loaders = [
    ...
    for i in range(cfg.share.num_splits - 1):
        ...
        if cfg.dataset.task == 'graph':
            ...
            loaders_j.append(
                get_loader(dataset_j[id], cfg.val.sampler, cfg.train.batch_size,
                           shuffle=False))
            delattr(dataset_j.data, split_names[i])
        else:
            loaders.append(
