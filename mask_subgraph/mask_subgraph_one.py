"""
@Name:  mask_subgraph_one.py
@Auth:  rongxing
@Date:  2023/4/12-下午3:02
@IDE:   PyCharm 
@PROJECT_NAME:   $ {PROJECT_NAME}
basis on mask_subgraph
[
def remove_subgraph
def mask_subgraph
]
alter content:
one owe original graph
other owe masked graph
"""
import csv
import math
import time
...
def mask_subgraph_one(smiles_item,Tm):
    ...
    x2 = torch.tensor(chirality_idx, dtype=torch.long).view(-1, 1)  ### must delete
    ...
    data_i_origin = Data(x=x1, edge_index=edge_index, edge_attr=edge_attr,
                      y=torch.Tensor([Tm]))
    ...
    # return data_i_origin, data_i_new
    return data_i_new, data_i_origin
