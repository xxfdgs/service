...
if __name__ == '__main__':
    ...
    dump_cfg(cfg) ### fully input data for cfg
    ...
    if cfg.train.mode == 'double':
        read_name = os.path.basename(cfg.out_dir)
        read_path = os.path.dirname(cfg.out_dir) + os.sep
        ...
        file_name = (cfg.out_dir).split('/')[1] #'gps_predict'
        save_path = cfg.save_path
        repeat_num = args.repeat
        picture_list = 4  # 4 # 1
        if cfg.property_num ==6:
            main_run(file_name, save_path,repeat_num,cfg.read_csv)
        elif cfg.property_num ==1:
            main_run_single(file_name, save_path, repeat_num, cfg.read_csv,cfg.property_serial)
