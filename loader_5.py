...
@Name:  loader_j.py
@Auth:  rongxing
@Date:  2023/2/18-上午10:33
@IDE:   PyCharm 
@PROJECT_NAME:   $ {PROJECT_NAME}
lrx add
one dataset transfer to two dataset ,such as dataset,dataset_j
"""
...
from graphgps.determinism import dataloader_generator, dataloader_worker_init
...
def load_dataset():
    ...
    for func in register.loader_dict.values():
        # dataset, dataset_j = func(format, name, dataset_dir)
        # if dataset and dataset_j is not None:
        #     return dataset, dataset_j
        dataset, dataset_2, dataset_3, dataset_4, dataset_5 = func(format, name, dataset_dir)
        if dataset and dataset_2 and dataset_3 and dataset_4 and dataset_5 is not None:
...
def set_dataset_info(dataset):
    ...
    try:
        ...
        # print('exception')
        cfg.share.dim_out = 1
    ### lrx set dim_out value
    # cfg.share.dim_out = cfg.gnn.dim_inner
...
def create_dataset():
    ...
    """
    # dataset, dataset_j = load_dataset()
    # set_dataset_info(dataset)
    # set_dataset_info(dataset_j)
    # return dataset, dataset_j
    dataset, dataset_2, dataset_3, dataset_4, dataset_5 = load_dataset()
    set_dataset_info(dataset)
    set_dataset_info(dataset_2)
    set_dataset_info(dataset_3)
    set_dataset_info(dataset_4)
    set_dataset_info(dataset_5)
    return dataset, dataset_2, dataset_3, dataset_4, dataset_5
...
def create_loader_5():
    ...
    """
    # dataset, dataset_j = create_dataset()
    dataset, dataset_2, dataset_3, dataset_4, dataset_5 = create_dataset()
    # train loader
    if cfg.dataset.task == 'graph':
        ...
        loaders_2 = [
            get_loader(dataset_2[id], cfg.train.sampler, cfg.train.batch_size,
                       shuffle=False)
        ]

        delattr(dataset_2.data, 'train_graph_index')
        loaders_3 = [
            get_loader(dataset_3[id], cfg.train.sampler, cfg.train.batch_size,
                       shuffle=False)
        ]
        delattr(dataset_3.data, 'train_graph_index')
        loaders_4 = [
            get_loader(dataset_4[id], cfg.train.sampler, cfg.train.batch_size,
                       shuffle=False)
        ]
        delattr(dataset_4.data, 'train_graph_index')
        loaders_5 = [
            get_loader(dataset_5[id], cfg.train.sampler, cfg.train.batch_size,
                       shuffle=False)
        ]
        delattr(dataset_5.data, 'train_graph_index')
    ...
    for i in range(cfg.share.num_splits - 1):
        ...
        if cfg.dataset.task == 'graph':
            ...
            loaders_2.append(
                get_loader(dataset_2[id], cfg.val.sampler, cfg.train.batch_size,
                           shuffle=False))
            delattr(dataset_2.data, split_names[i])
            loaders_3.append(
                get_loader(dataset_3[id], cfg.val.sampler, cfg.train.batch_size,
                           shuffle=False))
            delattr(dataset_3.data, split_names[i])
            loaders_4.append(
                get_loader(dataset_4[id], cfg.val.sampler, cfg.train.batch_size,
                           shuffle=False))
            delattr(dataset_4.data, split_names[i])
            loaders_5.append(
                get_loader(dataset_5[id], cfg.val.sampler, cfg.train.batch_size,
                           shuffle=False))
            delattr(dataset_5.data, split_names[i])


    return loaders, loaders_2, loaders_3, loaders_4, loaders_5
    # return loaders, loaders_j
